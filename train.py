...
def main():
    ...
    data.bethe_hessian_matrix = sparse.csr_matrix(
        (r ** 2 - 1) * np.eye(data.adj_matrix.shape[0]) - r * data.adj_matrix.toarray() + data.deg_in_matrix.toarray())
    ...
    try:
        ...
        for epoch in range(args.num_epochs):
            ...
            for iter in range(num_iterations):
                ...
                output = model.module(
                    x=sparse_mx_to_torch_sparse_tensor(
                        sparse_fill(shape=data.features.shape, mx=data.features[sample.input_nodes],
                                    row=sample.input_nodes)),
