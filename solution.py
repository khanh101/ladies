from utils import sparse_mx_to_torch_sparse_tensor, row_norm
...
def prepare_data(pool, sampler, process_ids, train_nodes, valid_nodes, samp_num_list, num_nodes, lap_matrix, depth):
    ...
    for _ in process_ids:
        ...
        idx = tf.random.shuffle(range(len(train_nodes)), tf.random.uniform([1]))[
            :args.batch_size]
        batch_nodes = train_nodes[idx]
        p = pool.apply_async(sampler, args=(np.random.randint(2**32 - 1), batch_nodes,
                                            samp_num_list, num_nodes, lap_matrix, depth))
        jobs.append(p)
    # idx = torch.randperm(len(valid_nodes))[:args.batch_size]
    idx = tf.random.shuffle(range(len(valid_nodes)), tf.random.uniform([1]))[
        :args.batch_size]
    batch_nodes = valid_nodes[idx]
    p = pool.apply_async(sampler, args=(np.random.randint(2**32 - 1), batch_nodes,
                                        samp_num_list * 20, num_nodes, lap_matrix, depth))
    jobs.append(p)
    return jobs
...
def ladies_sampler(seed, batch_nodes, samp_num_list, num_nodes, lap_matrix, depth):
    ...
    for d in range(depth):
        ...
        # This part not sure if it is calling row_normalize
...
if __name__ == "__main__":
    ...
    edges, labels, feat_data, num_classes, train_nodes, valid_nodes, test_nodes = load_data(
        'reddit')
    ...
    samp_num_list = np.array(
        [args.batch_size, args.batch_size, args.batch_size, args.batch_size, args.batch_size])
    ...
    susage = SuGCN(encoder=encoder, num_classes=num_classes,
                   dropout=0.5, inp=feat_data.shape[1])
    susage.to(device)
    ...
    for oiter in range(5):
        ...
        for epoch in np.arange(args.epoch_num):
            ...
            '''
            jobs = prepare_data(pool, sampler, process_ids, train_nodes, valid_nodes, samp_num_list, len(
                feat_data), lap_matrix, args.n_layers)
            ...
            adjs = package_mxl(adjs, device)
            output = susage.forward(feat_data[input_nodes], adjs)
            loss_valid = F.cross_entropy(
                output, labels[output_nodes]).detach().tolist()
            valid_f1 = f1_score(output.argmax(dim=1).cpu(),
                                labels[output_nodes].cpu(), average='micro')
        ...
        for b in np.arange(len(test_nodes) // args.batch_size):
            batch_nodes = test_nodes[b *
                                     args.batch_size: (b+1) * args.batch_size]
            ...
            loss_test = F.cross_entropy(
                output, labels[output_nodes]).cpu().detach().tolist()
            test_f1 = f1_score(output.argmax(dim=1).cpu(),
                               labels[output_nodes].cpu(), average='micro')
            test_f1s += [test_f1]
